...
class Home extends BaseController
{
	public function index()
	{
		...
		$data['new_products'] = $produsts->where('publication_status', '1')->orderBy('id', 'DESC')->findAll();
		...
		$data['featured_products'] = $produsts->where('product_feature', '1')->where('publication_status', '1')->orderBy('id', 'DESC')->limit(5, 1)->findAll();
		$data['discount'] = $discount->where('discount_status', '1')->findAll();
		return view('web/pages/home', $data );
	}

  public function single($product_id=null){
    $produsts = new ProductsModel();
		$data['single_product'] = $produsts->find($product_id);
